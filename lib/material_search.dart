...
typedef CustomerModel FormFieldFormatter<T>(T v);
typedef bool MaterialSearchFilter<T>(CustomerModel v, CustomerModel c);
typedef int MaterialSearchSort<T>(T a, T b, CustomerModel c);
typedef Future<List<MaterialSearchResult>> MaterialResultsFinder(CustomerModel c);
typedef void OnSubmit(CustomerModel value);
    ...
    return new Container(
      child: new Row(
        children: <Widget>[
          ...
          new Container(width: 70.0, child: new Text(text.code, style: Theme.of(context).textTheme.subhead)),
          new Expanded(child: new Text(text.name, style: Theme.of(context).textTheme.subhead)),
        ],
      ),
      height: 56.0,
    );
...
class _MaterialSearchState<T> extends State<MaterialSearch> {
  ...
  CustomerModel _criteria = new CustomerModel(name: '');
  TextEditingController _controller = new TextEditingController();

  _filter(CustomerModel v, CustomerModel c) {
    return v.name.toLowerCase().trim().contains(new RegExp(r'' + c.name.toLowerCase().trim() + ''));
  }
  ...
  void initState() {
    ...
    _controller.addListener(() {
      setState(() {
        _criteria.name = _controller.value.text;
        if (widget.getResults != null) {
          _getResultsDebounced();
        }
      });
        ...
        title: new TextField(
          ...
          onSubmitted: (String value) {
            if (widget.onSubmit != null) {
              widget.onSubmit(CustomerModel(name: value, nameLowercase: value.toLowerCase()));
            }
          },
        ),
        actions: _criteria.name.length == 0 ? [] : [
          new IconButton(
            icon: new Icon(Icons.clear),
            onPressed: () {
              setState(() {
                _controller.text = _criteria = null;
              });
            }
          ),
        ],
